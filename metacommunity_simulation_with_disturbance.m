function metacommunity_simulation_with_disturbance(in_param,output)
...
while(t(i)<t_max)
  ...
  rates(npatches+1) = dist_freq; #adds the disturbance rate to the vector of patch-specific rates
...
#if patch number npatches+1 is selected it means that disturbance happens

if select_patch == npatches+1
  #select patches to be disturbed
  p_select = 1:npatches;
  disturbed_patches = p_select(randperm(length(p_select))(1:dist_extent*npatches)); #select randomly dist_extent*npatches patches
  for p = disturbed_patches
    #kill a given proportion of individuals of all species (rounded to the nearest integer)
    x(p,:,i+1)=round(x(p,:,i+1).*(1-dist_intensity));
  endfor
  
  else #do birth, death or migration
